    ...
    if (was_outbound && acc->rfc5626_status == OUTBOUND_NA) {
        ...
        if (acc->regc && pj_strcmp(&prev_contact, &acc->reg_contact) != 0) {
            ...
            if (rc != PJ_SUCCESS) {
                ...
                /* set_contact() may already have moved the live Contacts to
                 * the expires=0 list before failing, which would leave the
                 * next refresh with none. The previous Contact parsed when
                 * it was built, so pushing it back restores a working
                 * registration even though it still advertises outbound.
                 */
